...
def check8Dfile(d_list: list, s_list: list):

   error_code = 0
   th_list = {'D0': 1, 'D1': 50, 'D2': 1, 'D3': 50, 'D4': 50, 'D5': 50,
              'D6': 50, 'D7': 50, 'D8': 50, 'Action': 50}

   for iteration in s_list:
       if len(d_list[iteration].index) > th_list[iteration]:
           error_code = iteration
...
class DocProcess:
   ...
   def process8Dfile(self):
       ...
       d_list['D4'] = d_list['D4'].drop(columns=d_list['D4'].columns[0])

       blob = read_file(self.file_path)
       d_list['file'] = blob

       return check8Dfile(d_list, s_list), d_list, s_list
